"""
CrowdSense AI - ESP32 OLED + LED Display Controller (USB)
Fetches data from deployed app and sends to ESP32 via USB
"""
...
# What to display
QUERY_PARAM = "q=12b"  # q=12b (bus), q=apollo (hospital), q=meenakshi (temple)
...
def fetch_data(query):
    ...
    try:
        url = f"{SERVER_URL}/api/bus?{query}"
...
def send_to_esp32(ser, data):
    ...
    if data:
        ...
        try:
            ...
            # Also print what LEDs will light up
            if data['status'] == 'High':
                print(">>> RED LED will ON")
            elif data['status'] == 'Medium':
                print(">>> YELLOW LED will ON")
            elif data['status'] == 'Low':
                print(">>> GREEN LED will ON")
...
def main():
    ...
    ser = connect_serial()
    if not ser:
        print("\nFailed to connect to ESP32. Make sure:")
    ...
    print("OLED Display + Traffic LEDs will update")
    print("Press Ctrl+C to stop\n")
    
    try:
        while True:
            data = fetch_data(QUERY_PARAM)
            if data:
